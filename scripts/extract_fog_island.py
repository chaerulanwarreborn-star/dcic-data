...
OUTPUT_PATH = DIST_DIR / "fog_island.json"
...
def find_localization_path() -> Path:
    ...
    canonical = LOCALIZATION_DIR / "dragon_city_localization_baseline_en.json"
    if canonical.exists():
        return canonical
    ...
    fallbacks = [
        REPO_ROOT / "dragon_city_localization_baseline_en.json",
    ]
    for path in fallbacks:
        if path.exists():
            return path

    matches = sorted(LOCALIZATION_DIR.glob("*localization*en*.json")) if LOCALIZATION_DIR.exists() else []
    if matches:
        return matches[0]
