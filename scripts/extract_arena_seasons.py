...
ROOT = Path(__file__).resolve().parent           # .../dcic-data/scripts
REPO_ROOT = ROOT.parent                            # .../dcic-data
DIST_DIR = REPO_ROOT / "dist"
OVERRIDES_DIR = REPO_ROOT / "overrides"
LOCALIZATION_DIR = REPO_ROOT / "localization"
# Raw game-data captures now live in a separate (private) repo, expected as a
# sibling folder next to dcic-data by default. Override with the DCIC_RAW_DIR
# env var (e.g. in CI, where the private repo may be checked out elsewhere).
RAW_DIR = Path(os.environ.get("DCIC_RAW_DIR", REPO_ROOT.parent / "the-void"))

ARENA_CONFIG_PATH = RAW_DIR / "arena_config.json"
GAME_CONFIG_PATH = RAW_DIR / "game_config.json"
DRAGONS_PATH = DIST_DIR / "dragons.json"
LOCALIZATION_PATH = LOCALIZATION_DIR / "dragon_city_localization_baseline_en.json"
OVERRIDES_PATH = OVERRIDES_DIR / "arena_overrides.json"
OUTPUT_PATH = DIST_DIR / "arena_seasons.json"
